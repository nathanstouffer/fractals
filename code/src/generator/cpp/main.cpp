...
#include "generator/fractals.hpp"
...
namespace generator
{
    ...
    rgb_t pixel_color(fractal* frac, unsigned int i, unsigned int j)
    {
        int r = 0;
        int g = 0;
        int b = 0;
        ...
        for (size_t u = 0; u < SAMPLES_RESOLUTION; ++u)
        {
            for (size_t v = 0; v < SAMPLES_RESOLUTION; ++v)
            {
                ...
                rgb_t color = frac->color_complex_num(num);
                r += color.r;
                g += color.g;
                b += color.b;
            }
     ...
     */
    void color_pixels(bitmap_image* image, fractal* frac, unsigned int i, int min_j, int max_j)
    {
        for (unsigned int j = min_j; j < max_j; j++)
        {
            rgb_t color = pixel_color(frac, i, j);
            image->set_pixel(i, j, make_colour(color.r, color.g, color.b));
        }
    }

    void render(bitmap_image* image, fractal* frac) {
        ...
        for (unsigned int i = 0; i < WIDTH; i++)
        {
            ...
            for (unsigned int t = 0; t < NUMTHREADS; ++t)
            {
                ...
                threads.push_back(std::thread(color_pixels, image, frac, i, min, max));
            }
...
int main(int argc, char** argv) {
    ...
    generator::Types option = generator::Types::MANDELBROT;

    // create an image 640 pixels wide by 480 pixels tall
    bitmap_image tmp(WIDTH, HEIGHT);
    bitmap_image* image = &tmp;
    generator::rgb_t conv = { CONV };

    // Mandelbrot set
    generator::mandelbrot mand(conv, RED, GREEN, BLUE);
    // Teardrop (inverted mandelbrot) - we rotate around the x-axis by theta
    double theta = M_PI;
    generator::teardrop teardrop(theta, conv, RED, GREEN, BLUE);
    // tetration (power tower)
    generator::powertower power(conv, RED, GREEN, BLUE);
    // newton's method in the complex plane
    generator::newton newt(conv);

    // set up fractal gen pointer
    generator::fractal* fractal = nullptr;

    // render the fractal
    switch (option) {
        case generator::Types::MANDELBROT:
            fractal = &mand;
            std::cout << "Starting mandelbrot" << std::endl;
            generator::render(image, fractal);
            image->save_image("../../../../img/mandelbrot/mandelbrot.bmp");
            std::cout << "Success for mandelbrot" << std::endl;
            break;
        case generator::Types::TEARDROP:
            fractal = &teardrop;
            ...
            break;
        case generator::Types::POWERTOWER:
            fractal = &power;
